...
def make_ConvNet2D_model():
  ...
  """
  return snt.Sequential([
      snt.nets.ConvNet2D([LATENT_SIZE] * NUM_LAYERS, activate_final=True),
      snt.LayerNorm()
  ])
...
class ConvNet2DGraphNetwork(snt.AbstractModule):
  """GraphNetwork with MLP edge, node, and global models."""

  def __init__(self, name="MLPGraphNetwork"):
    super(ConvNet2DGraphNetwork, self).__init__(name=name)
    with self._enter_variable_scope():
      self._network = modules.GraphNetwork(make_ConvNet2D_model(), make_ConvNet2D_model(),
                                           make_ConvNet2D_model())
